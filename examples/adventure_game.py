...
class AdventureGame:
    ...
    def _simulate_game_actions(self) -> None:
        ...
        # Process all events that were generated by handlers before ending the game
        # We need to do this because some events create future events in later ticks
        # This ensures we don't have events after the game.end event
        self._process_pending_events([hallway_event.id, key_event.id, chamber_event.id, treasury_event.id])
    ...
    def _process_pending_events(self, skip_ids: List[str]) -> None:
        """Process all pending events in the log that haven't been handled yet.
        
        Args:
            skip_ids: List of event IDs to skip (already processed)
        """
        for tick in range(self.event_log.current_tick + 1):
            ...
            for event in events_in_tick:
                # Skip if game is over or if event was already processed
                if self.game_over or event.id in skip_ids or event.type == "game.start":
                    continue
                    
                # Call appropriate handler based on event type
                if event.type == "player.move":
                    self._handle_player_move(event)
                elif event.type == "player.pickup_item":
                    self._handle_pickup_item(event)
                elif event.type == "room.enter":
                    ...
                    if "item" in event.data and not self.game_over:
                        self.player_inventory.append(event.data["item"])
                        
                # Add this event ID to the skip list so we don't process it again
    ...
    def _handle_pickup_item(self, event: Event) -> None:
        ...
        if item == "health_potion":
            health_event: Event = self.event_log.add_event("player.health_change", {
                "amount": 20,
                "reason": "Used health potion"
            }, parent_event=event)
            self._handle_health_change(health_event)
        elif item == "torch":
            # Torch improves visibility, add future discoveries
            map_event: Event = self.event_log.add_event("item.discover", {
                "item": "hidden_map",
    ...
    def _handle_enemy_encounter(self, event: Event) -> None:
        ...
        combat_start_event: Event = self.event_log.add_event("combat.start", {
            "enemy": enemy,
        ...
        round1_event: Event = self.event_log.add_event("combat.round", {
            "enemy": enemy,
            "round": 1,
            "message": f"Round 1 against {enemy}!"
        }, parent_event=event)
        self._handle_combat_round(round1_event)
        
        # Add two more combat rounds if game is not over yet
        if not self.game_over:
            # Add round 2
            round2_event: Event = self.event_log.add_event("combat.round", {
                "enemy": enemy,
                "round": 2,
                "message": f"Round 2 against {enemy}!"
            }, parent_event=event)
            self._handle_combat_round(round2_event)
            
            # Only proceed with round 3 if game is not over
            if not self.game_over:
                round3_event: Event = self.event_log.add_event("combat.round", {
                    "enemy": enemy,
                    "round": 3,
                    "message": f"Round 3 against {enemy}!"
                }, parent_event=event)
                self._handle_combat_round(round3_event)
    ...
    def _handle_combat_round(self, event: Event) -> None:
        ...
        # Add damage events
        self.event_log.add_event("combat.damage_dealt", {
            "enemy": enemy,
            "amount": damage_dealt,
            "message": f"You hit the {enemy} for {damage_dealt} damage!"
        }, parent_event=event)
        
        self.event_log.add_event("combat.damage_taken", {
            "enemy": enemy,
            "amount": damage_taken,
        ...
        if round_num == 3:
            self.event_log.add_event("enemy.defeated", {
                "enemy": enemy,
    ...
    def _handle_health_change(self, event: Event) -> None:
        ...
        if self.player_health <= 0:
            # Add player death event
            death_event: Event = self.event_log.add_event("player.death", {
    ...
    def _handle_game_end(self, event: Event) -> None:
        ...
        # Add final score event to the log
        score: int = (
        ...
        self.event_log.add_event("game.final_score", {
            "score": score,
