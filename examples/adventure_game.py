...
class AdventureGame:
    ...
    def start_game(self) -> None:
        ...
        self.event_bus.publish("player.move", {
    ...
    def _simulate_game_actions(self) -> None:
        ...
        # Pick up key
        self.event_log.advance_tick()
        self.event_bus.publish("player.pickup_item", {
            "item": "key",
    ...
    def _handle_player_move(self, event: Event) -> None:
        ...
        self.event_bus.publish("room.enter", {
            "room": destination,
    ...
    def _handle_room_enter(self, event: Event) -> None:
        ...
        for enemy in room_data["enemies"]:
            ...
            if random.random() > 0.3:
                self.event_bus.publish("enemy.encounter", {
                    "enemy": enemy,
        ...
        for treasure in room_data["treasures"]:
            ...
            if random.random() > 0.7:
                self.event_bus.publish("treasure.found", {
                    "treasure": treasure,
    ...
    def _log_event(self, event: Event) -> None:
        """Log all events to the event log.
        
        This is a special handler that subscribes to all events via the '*' wildcard
        and ensures they are added to the event log for visualization and querying.
        """
        # Add the event to the log if it's not already there
        # We check the ID to avoid duplicating events
        if not any(e.id == event.id for e in self.event_log.events):
